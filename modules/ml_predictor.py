...
def ensemble_predict(X_train, y_train, X_test, time_limit=1200, hyperparameters=None):
    ...
    try:
        ...
        if not isinstance(y_train, pd.Series):
            if isinstance(y_train, pd.DataFrame):
                ...
                if y_train.shape[1] == 1:
                    ...
                    # Use the first column with a warning
                    logger.warning(f"y_train is a DataFrame with multiple columns, using first column only")
                    y_train = y_train.iloc[:, 0]
                ...
                if y_train.ndim > 1:
                    if y_train.shape[1] == 1:
                        ...
                        logger.warning(f"y_train is a 2D array with multiple columns, using first column only")
                        y_train = y_train[:, 0]
        ...
        if isinstance(predictions, pd.Series):
            ...
            # If predictions is a DataFrame, convert to Series then to numpy array
            if predictions.shape[1] == 1:
                predictions = predictions.iloc[:, 0].values
            else:
                logger.warning(f"Predictions DataFrame has multiple columns, using first column only")
                predictions = predictions.iloc[:, 0].values
        
        # Ensure predictions is a 1D array (explicitly flatten if necessary)
        if isinstance(predictions, np.ndarray) and predictions.ndim > 1:
            logger.info(f"Flattening predictions array from shape {predictions.shape}")
...
def ensemble_predict_wrapper(X_train, y_train, X_test, time_limit=1200, hyperparameters=None):
    ...
    """
    try:
        # Call ensemble_predict directly with conversion handling
        # Ensure y_train is a Series if it's a DataFrame
        if isinstance(y_train, pd.DataFrame):
            if y_train.shape[1] == 1:
                logger.info("Converting y_train from DataFrame to Series")
                y_train = y_train.iloc[:, 0]
